...
dblong_day <- df_cut_times(dblong,1,"1 day","ymd HMS")
...
temp_env_1hour <-temp_env_1hour %>%
  group_by(Date_time) %>%
  summarise(Hs_1hour_mean=mean(Hs.m.))
...
#wide bivalves
dbwide_hour <- dblong/_hour %>% select(Date_time,Deployment.number,Bivalve.number,mean.biv.gap.precent) %>%
  spread(key = Bivalve.number,value = mean.biv.gap.precent)
#wide environment
environment_wide <- dblong_hour %>% select(Date_time,Deployment.number,Hs.m.,Tp.s.,Dp.deg.,H1.10.m.,Tmean.s.,Current.Speed.cm.sec.,Current.Direction.deg.,Temp_1hour_mean,Lux_1hour_mean,Turbidity_1hour_mean) %>% 
  distinct()
...
hist(env_corr$corr_values[env_corr$environmental_factor=="Turbidity_1houR" & env_corr$p_values<0.05],breaks = seq(-1,1,0.1))
write.csv(env_corr,"Environmental_correlations_all_duration.csv")
...
for (biv_num in bivalves_id) {
    ...
    temp_biv <- merge(temp_biv,environment_week_wide,by="group",all.x=T)
    #remove datetime
    temp_biv <- temp_biv %>% select(-Deployment.number,-group)
    #getting correlation, p-values and n and convert them to data frame
    temp_corr <- rcorr(as.matrix(temp_biv),type = "pearson")
    temp_corr <-data.frame(biv_num=biv_num,
                       #starting from 2 to avoid autocorrelation with the bivalve
...
hist(env_corr_week_significant$corr_values,seq(-1,1,by=0.1))
...
temp_corr <- biv_mean_daily %>% select(-group,-n_bivalves,-Date_time,-daily_mean_biv_open_degree)
...
env_corr_day$corr_values[1:4] <- temp_corr[["r"]][6,1:4]
env_corr_day$p_values[1:4] <- temp_corr[["P"]][6,1:4]
env_corr_day$n[1:4] <- temp_corr[["n"]][6,1:4]
env_corr_day$corr_values[5:8] <- temp_corr[["r"]][5,1:4]
env_corr_day$p_values[5:8] <- temp_corr[["P"]][5,1:4]
env_corr_day$n[5:8] <- temp_corr[["n"]][5,1:4]
rm(temp_corr)
