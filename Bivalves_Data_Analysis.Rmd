...
hist(biv_corr_day$corr_values,breaks = seq(-1,1,0.1))
```
...
##check mean and max opening time for in 3 hours bins
...
for (biv in bivalves_names) {
  ...
  for (date in temp_dates_list) {
    ...
    bivalves_3h_open_time_ratio_and_perc <- rbind(bivalves_3h_open_time_ratio,temp_date)
  }
}time
rm(temp_date,temp_biv)
write.csv(bivalves_3h_open_time_ratio,"Bivalves_3h_open_time&perc_ratio.csv")
sum_bivalves_3h_open_time_ratio_and_perc <- bivalves_3h_open_time_ratio %>% 
