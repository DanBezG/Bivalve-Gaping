...
```{r}
tal_spring <- read.csv("Gaping data Tal Amit Fall.csv")
#Filtering only the right rows and columns
...
tal_sum_by_hour <- tal_spring_new[tal_spring_new$Open.precentage==1,]
tal_sum_by_hour <- tal_sum_by_hour %>%group_by(bivalve.code,Hour,Date) %>%  summarise(open_hour_ratio=sum(hour_ratio))
write.csv(tal_sum_by_hour,"Gaping data Tal Amit Fall_sum_by_hour.csv")


tal_fall <- read.csv("Gaping data Tal Amit Fall.csv")
tal_fall <- tal_fall[1:495,]
```
