...
bivalves_3h_open_time_ratio <- data.frame(
  bivalve_name=character(),
  date=Date(),
  hour=character(),
  open_time_ratio=double(),
...
for (biv in bivalves_names) {
  ...
  for (date in temp_dates_list) {
    ...
    temp_date <- temp_date %>% group_by(ThreeHour_bin) %>%
      summarise(Open_time_ratio=sum(open_close=="Open")/n(),n=n(),season=unique(Season)) 
    ...
    bivalves_3h_open_time_ratio <- rbind(bivalves_3h_open_time_ratio,temp_date)
  }
}
rm(temp_date,temp_biv)
write.csv(bivalves_3h_open_time_ratio,"Bivalves_3h_open_time_ratio.csv")
sum_bivalves_3h_open_time_ratio <- bivalves_3h_open_time_ratio %>% 
