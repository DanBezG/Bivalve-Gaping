...
biv_df_cut_times <- function(biv_df,date_col_num,break_interval,date_format) {
  ...
  summarise(biv_open_interval_perc=mean(biv_open_percent),biv_open_interval=mean(biv_open),per99.9=mean(percentile.99),biv_open_perc_logit=mean(logit_transformation))
  return(biv_df_cut_time)
}

GetSeason <- function(date) {
  Spring_start_day <- 60
  Summer_start_day <- 152
  Fall_start_day <- 244
  Winter_start_day <- 335
  season_vecotr <- vector(mode = "character",length = length(date)) 
  for (dat in 1:length(date)) {
  season_vecotr[dat] <- ifelse (date[dat] >= Spring_start_day & date[dat] < Summer_start_day, "Spring",
      ifelse (date[dat] >= Summer_start_day & date[dat] < Fall_start_day, "Summer",
        ifelse (date[dat] >= Fall_start_day & date[dat] < Winter_start_day, "Fall", "Winter")))
  
  }
  return(season_vecotr)
}

logit <- function(percentage) {
    percentage <- ifelse(percentage==100,percentage-0.1,ifelse(percentage==0,percentage+0.1,percentage))
    prob <- percentage/100
    odd_precent <- (prob)/(1-prob)
    logit <- log(odd_precent)
    return(logit)
}

split_rows_by_hours_tal <- function(row) {
   start_hour <- as.POSIXlt(row$Time.Start, format="%H:%M:%S")$hour
   end_hour <- as.POSIXlt(row$Time.end, format="%H:%M:%S")$hour
  
  if (start_hour != end_hour) {
    # Create new rows with adjusted time for each hour within the activity duration
      new_rows <- lapply(seq(start_hour, end_hour,by = 1), function(hour) {
      row_copy <- row
      row_copy$Time.Start <- if (hour == start_hour) row$Time.Start else paste(sprintf("%02d", hour), "00", "00", sep=":")
      row_copy$Time.end <- if (hour == end_hour) row$Time.end else paste(sprintf("%02d", hour), "59", "59", sep=":")
      return(row_copy)
    })
...
```{r}
col_names<-colnames(db)[3:dim(db)[2]]
...
dblong$biv_open_percent <- ifelse(dblong$biv_open_percent>100,100,dblong$biv_open_percent)
#calculate logit
dblong$logit_transformation <- logit(dblong$biv_open_percent)
...
write.csv(dblong_week, "Big_Hobo_databse_long_day_percent.csv")
...
names(dblong_hour)<-c("Date_time","Deployment.number","Bivalve.number","mean.biv.gap.precent","mean.biv.open.degree","percentile.99","logit_transformation","Hs[m]","Tp[s]","Dp[deg]","H1/10[m]","Tmean[s]","Current.Speed[cm/sec]","Current.Direction[deg]")
...
db_model <- dblong_hour %>% select(Date_time,Deployment.number,Bivalve.number,mean.biv.gap.precent,logit_transformation,`Hs[m]`,`Current.Speed[cm/sec]`,Temp_1hour_mean,Turbidity_1hour_mean,Lux_1hour_mean)
saveRDS(db_model,"Bivalves_model_df.RDS")
```
...
env_1hour_to_day <- dblong_hour %>% 
  select(Date_time,`Hs[m]`,`Current.Speed[cm/sec]`) %>% 
  distinct()
env_1hour_to_day$Date_time <- parse_date_time(env_1hour_to_day$Date_time,orders = "ymd HMS")
env_1hour_to_day$Date_time <- cut(env_1hour_to_day$Date_time, breaks= "1 day")
env_1hour_to_day$`Hs[m]` <- as.numeric(env_1hour_to_day$`Hs[m]`)
env_1hour_to_day$`Current.Speed[cm/sec]` <- as.numeric(env_1hour_to_day$`Current.Speed[cm/sec]`)
env_1hour_to_day <-env_1hour_to_day %>%
  group_by(Date_time) %>%
  summarise(Hs_1day_mean=mean(`Hs[m]`,na.rm=T))
env_1day <- merge(env_1day,env_1hour_to_day,by="Date_time")
rm(env_1hour_to_day)
```
...
reco <- reco %>%filter(Station=="ST3") %>%
  select(group,Ex..Chl...ug.L..new,Depth..m.) %>% 
  group_by(group)
#summarise to all depths
reco <- reco %>% 
  summarise(Chl=mean(Ex..Chl...ug.L..new,na.rm=T)) %>% 
  arrange(group)
...
temp_sun <- temp_sun %>% left_join(temp_night,by="group") %>%
...
dbwide2 <- dblong2 %>% select(group,Deployment.number,Biv_num,biv_open_perc_logit) %>% 
  spread(key = Biv_num,value = biv_open_perc_logit)
write.csv(dbwide2,"Big_Hobo_database_wide_2min_logit.csv")
deployments <- unique(dbwide2$Deployment.number)
#correlation between each bivalve pair in a singe deployment
...
temp_corr <- rcorr(as.matrix(temp_dep),type = "spearman")
...
write.csv(bivalve_corr,"Bivalves_correlations_spearman.csv")
hist(biv_corr_hour$corr_values,breaks = seq(-1,1,0.1))

biv_corr_all_res <- rbind(bivalve_corr,biv_corr_hour,biv_corr_day)
biv_corr_sum <- biv_corr_all_res %>% 
  group_by(res) %>% 
  summarise_at(vars(corr_values), funs(mean, median, sd, n())) %>% 
  mutate(conf_int = sd/sqrt(n) * qt(p = 0.975, df = n-1))
write.csv(biv_corr_sum,"Biv_corr_sum.csv")
...
#wide bivalves
dbwide_hour <- dblong_hour %>% select(Date_time,Deployment.number,Bivalve.number,logit_transformation) %>%
  spread(key = Bivalve.number,value = logit_transformation)
#wide environment
...
for (biv_num in bivalves_id) {
  ...
  #getting correlation, p-values and n and convert them to data frame
  temp_corr <- rcorr(as.matrix(temp_biv),type = "spearman")
...
hist(env_corr$corr_values[env_corr$environmental_factor=="Temp_1hour_mean"],breaks = seq(-1,1,0.1))
write.csv(env_corr,"Environmental_correlations_hour_all_duration_spearman_logit.csv")


##Environmental correlations - weekly (chl) 
#wide bivalves
dbwide_week <- dblong_week %>% select(group,Deployment.number,Biv_num,biv_open_perc_logit) %>%
  spread(key = Biv_num,value = biv_open_perc_logit)
#prepare the big data frame
...
rm(temp_corr)
write.csv(env_corr_week,"Chl_correlations_week-all_duration_spearman_logit.csv")
...
```
##Bivalve opening times(how much time was open in whole duration/daily/hourly)
```{r}
###check mean and max opening time for each bivalve for the whole deployments duration
bivalves_names<-unique(as.character(dblong2$Biv_num))
...
opening_interval_sum<- bivalves_opening_times %>% 
 ...
 summarise(biv_open_interval_mean=mean(mean.opening.time),sd_opening_time=sd(mean.opening.time),n=n(),biv_total_open_perc=mean(total_time_open),sd.total_time_open=sd(total_time_open)) %>% 
  mutate(conf_int_open_interval = sd_opening_time/sqrt(n) * qt(p = 0.975, df = n-1) )
write.csv(opening_interval_sum,"Bivalves_opening_interval_summary_by_limit.csv")
...
##check mean opening ratio for in 1 hours bins for 20 precent closing
dblong_1h_bin <- dblong2 %>% select(-Sunset,-Sunrise)
dblong_1h_bin$Day_bin <- cut(dblong_1h_bin$group, breaks= "1 day")
#chose 20 just to see if works
close_limit <- 20
dblong_1h_bin$DayofYear <- yday(dblong_1h_bin$group)
dblong_1h_bin$Season <- GetSeason(dblong_1h_bin$DayofYear)
dblong_1h_bin$open_close <- ifelse(dblong_1h_bin$biv_open_interval_perc>close_limit,"Open","Close")
bivalves_1h_open_time_ratio_and_perc <- data.frame(
  bivalve_name=character(),
  date=Date(),
  hour=character(),
  open_time_ratio=double(),
  mean_open_perc=double(),
  n=integer(),
  Season=character()
)
for (biv in bivalves_names) {
  temp_biv <- dblong_1h_bin %>% filter(Biv_num==biv)
  temp_dates_list <-unique(temp_biv$Day_bin)
  for (date in temp_dates_list) {
    temp_date <- temp_biv %>% filter(Day_bin==date)
    temp_date$Hour_bin <-  cut(temp_date$group, breaks= "1 hours")
    temp_date <- temp_date %>% group_by(Hour_bin) %>%
      summarise(Open_time_ratio=sum(open_close=="Open")/n(),n=n(),season=unique(Season),mean_open_perc = mean(biv_open_interval_perc)) 
    temp_date <- data.frame(
                 bivalve_name=biv,
                 date=date,
                 hour=format(ymd_hms(temp_date$ThreeHour_bin),"%H:%M"),
                 open_time_ratio=temp_date$Open_time_ratio,
                 mean_open_perc=temp_date$mean_open_perc,
                 n=temp_date$n,
                 Season=temp_date$season)
    bivalves_1h_open_time_ratio_and_perc <- rbind(bivalves_1h_open_time_ratio_and_perc,temp_date)
  }
}
rm(temp_date,temp_biv)
write.csv(bivalves_1h_open_time_ratio_and_perc,"Bivalves_1h_open_time&perc_ratio.csv")
sum_bivalves_1h_open_time_ratio_and_perc <- bivalves_1h_open_time_ratio_and_perc %>% 
  group_by(hour,Season) %>% 
  summarise_at(vars(open_time_ratio,mean_open_perc), funs(mean, median, sd, n())) %>% 
  mutate(conf_int = sd/sqrt(n) * qt(p = 0.975, df = n-1))
write.csv(sum_bivalves_1h_open_time_ratio,"Bivalves_1h_open_time_ratio_sum.csv")

```

##Monthly Environemntal data
```{r}
env_10min_to_month <- dblong10 %>% 
  select(Temp,lux,Turbidity,group) %>% 
  distinct()
env_10min_to_month$group <- parse_date_time(env_10min_to_month$group,orders = "ymd HMS")
env_10min_to_month$Date_time <- cut(env_10min_to_month$group, breaks= "1 month")
env_10min_to_month$Temp <- as.numeric(env_10min_to_month$Temp)
env_10min_to_month$lux <- as.numeric(env_10min_to_month$lux)
env_10min_to_month$Turbidity <- as.numeric(env_10min_to_month$Turbidity)
env_10min_to_month <-env_10min_to_month %>%
  group_by(Date_time) %>%
  summarise_at(vars(Temp,lux,Turbidity), funs(mean(.,na.rm=T),n(),sd(.,na.rm=T)))
env_1hour_to_month <- dblong_hour %>% 
  select(Date_time,`Hs[m]`,`Current.Speed[cm/sec]`) %>% 
  distinct()
env_1hour_to_month$Date_time <- parse_date_time(env_1hour_to_month$Date_time,orders = "ymd HMS")
env_1hour_to_month$Date_time <- cut(env_1hour_to_month$Date_time, breaks= "1 month")
env_1hour_to_month$`Hs[m]` <- as.numeric(env_1hour_to_month$`Hs[m]`)
env_1hour_to_month$`Current.Speed[cm/sec]` <- as.numeric(env_1hour_to_month$`Current.Speed[cm/sec]`)
env_1hour_to_month <-env_1hour_to_month %>%
  group_by(Date_time) %>%
  summarise_at(vars(`Hs[m]`,`Current.Speed[cm/sec]`), funs(mean(.,na.rm=T),n(),sd(.,na.rm=T)))
env_month <- merge(env_10min_to_month,env_1hour_to_month,by="Date_time")

env_1week_to_month <- dblong_week %>% select(group,Chl) %>% distinct()
env_1week_to_month$Date_time <- parse_date_time(env_1week_to_month$group,orders = "ymd")
env_1week_to_month$Date_time <- cut(env_1week_to_month$Date_time, breaks= "1 month")
env_1week_to_month$Chl <- as.numeric(env_1week_to_month$Chl)
env_1week_to_month <-env_1week_to_month %>%
  group_by(Date_time) %>%
  summarise_at(vars(Chl), funs(mean(.,na.rm=T),n(),sd(.,na.rm=T)))
env_month <- merge(env_month,env_1week_to_month,by="Date_time")
names(env_month)[17:19] <- c("Chl","Chl_n","Chl_sd")
write.csv(env_month,"Environmental_data_by_month.csv")

```

##Tal Amit's Data analysis
```{r}
tal_spring <- read.csv("Gaping data Tal Amit_Fall.csv")
#Filtering only the right rows and columns - only relevent to fall
tal_spring <- tal_spring[1:495,]
#for spring
tal_spring <- tal_spring %>% select(-Sunset.time,-Sunrise.time,-Time.from.sunrise)
#for fall
tal_spring <- tal_spring %>% select(-Leg.in.the.day,-duration,-Percentage.from.the.time)
tal_spring$Date <- as.Date(tal_spring$Date,format="%d.%m.%Y")

#new df with distinct hours
tal_spring_new <-data.frame(Date=Date(),
                            Time.Start = character(),
                            Time.end = character(),
                            Hour.Start=integer(),
                            Hour.End=integer(),
                            duration = character(),
                            bivalve.code = integer(),
                            status=integer(),
                            Open.precentage = double(),
                            Comments=character())
#fill in the new df
for (biv_num in unique(tal_spring$bivalve.code)) {
  temp_biv <- tal_spring %>% filter(bivalve.code==biv_num)
  for (day in unique(temp_biv$Date)) {
    temp_date <- temp_biv %>% filter(Date == day)
    for (row_num in 1:dim(temp_date)[1]) {
    
      temp_row <- split_rows_by_hours_tal(temp_date[row_num,])
      tal_spring_new <- rbind(tal_spring_new,temp_row)
    }                              
  }
}
tal_spring_new$Time.Start <- as.POSIXct(tal_spring_new$Time.Start, format = "%H:%M:%S")
tal_spring_new$Time.end <- as.POSIXct(tal_spring_new$Time.end, format = "%H:%M:%S")
#calculate duration and ratio from hour
tal_spring_new$Duration <- difftime(tal_spring_new$Time.end, tal_spring_new$Time.Start,units = "mins")
tal_spring_new$hour_ratio <- as.numeric(tal_spring_new$Duration)/60
tal_spring_new$Hour <- format(tal_spring_new$Time.Start,format = "%H")
tal_spring_new$ThreeHour_bin <-  cut(tal_spring_new$Time.Start, breaks= "3 hours")
tal_spring_new$ThreeHour_ratio <- as.numeric(tal_spring_new$Duration)/180
#Filter the wide open and sum
tal_open <- tal_spring_new[tal_spring_new$status==1 ,]
tal_sum_by_hour <- tal_open %>%group_by(bivalve.code,Hour,Date) %>%  summarise(open_hour_ratio=sum(hour_ratio))
tal_sum_by_threehour <- tal_open %>%group_by(bivalve.code,ThreeHour_bin,Date) %>%  summarise(open_hour_ratio=sum(ThreeHour_ratio))
tal_sum_of_hours <- tal_sum_by_hour %>%
  group_by(Hour) %>%
  summarise_at(vars(open_hour_ratio),funs(mean,median,n(),sd)) %>% 
  mutate(conf_int = sd/sqrt(n) * qt(p = 0.975,df = n-1))
tal_sum_of_3hours <- tal_sum_by_threehour %>%
  group_by(ThreeHour_bin) %>%
  summarise_at(vars(open_hour_ratio),funs(mean,median,n(),sd)) %>% 
  mutate(conf_int = sd/sqrt(n) * qt(p = 0.975,df = n-1))

excel <- createWorkbook()
addWorksheet(excel,sheetName = "Gaping data Tal Amit Fall_new")
addWorksheet(excel,sheetName = "Gaping sum by hour")
addWorksheet(excel,sheetName = "Gaping sum by 3hour")
addWorksheet(excel,sheetName = "Gaping 1 hour stat")
addWorksheet(excel,sheetName = "Gaping 3 hour stat")
writeData(excel,sheet = "Gaping data Tal Amit Fall_new",x = tal_spring_new)
writeData(excel,sheet = "Gaping sum by hour",x = tal_sum_by_hour)
writeData(excel,sheet = "Gaping sum by 3hour",x = tal_sum_by_threehour)
writeData(excel,sheet = "Gaping 1 hour stat",x = tal_sum_of_hours)
writeData(excel,sheet = "Gaping 3 hour stat",x = tal_sum_of_3hours)
saveWorkbook(excel,"Gaping data Tal Amit Fall_new.xlsx")
```

##FFT calc
```{r}
x <- fft(dblong2$biv_open_perc_logit[dblong2$Biv_num=="Bivalve.0"])
plot(Mod(x), type = "l", main = "FFT Magnitudes")
which.max(abs(x))
abs(x)[5]
...
##Open_time_ratio & open_perc seasonal plot
```{r}
season_open_time_graph <- ggplot(data = sum_bivalves_1h_open_time_ratio_and_perc, aes(x = hour, y = mean,color=Season,group=Season))+
  geom_line(size=1.5)+
  geom_ribbon(aes(ymin=mean-conf_int,ymax=mean+conf_int,fill=Season),color="black",alpha=0.1)+
...
season_open_perc_graph <- ggplot(data = sum_bivalves_1h_perc, aes(x = hour, y = mean,color=Season,group=Season))+
  geom_line(size=1.5)+
  geom_ribbon(aes(ymin=mean-conf_int,ymax=mean+conf_int,fill=Season),color="black",alpha=0.1)+
  scale_y_continuous(limits = c(0,1),breaks = seq(0,1,by=0.1))+
  labs(x = "Time of Day",
  ...
  theme(panel.grid.major = element_blank(),
        ...
        strip.text = element_text(size = 14),
        legend.text = element_text(size = 14))
open_perc_time_ratio_season_graph <-season_open_time_graph/season_open_perc_graph 
```

##Day_night plot
```{r}
#Day Night difference -  Works good!


Day_Night_gaping_graph_zoom <- ggplot(dblong2, aes(x = diff_sunset, y = 
                                                  biv_open_interval_perc,color=as.factor(Day_Night))) +
    # geom_point(size = 0.1, alpha = 0.2) +
    # geom_smooth( size = 1,color = "black",se =T,fill="gray")+
    geom_spline(size = 1,color = "black")+
    geom_vline(xintercept = 0,linetype="dashed",color="black",size = 1)+
    coord_cartesian(ylim = c(30,50),xlim = c(-12,12))+
...
Day_Night_gaping_graph_full <- ggplot(dblong2, aes(x = diff_sunset
                                                   , y = 
                                                  biv_open_interval_perc,color=as.factor(Day_Night))) +
    geom_point(size = 0.1, alpha = 0.2) +
    geom_smooth( size = 1.5,color = "black",se =T,fill="yellow")+
    geom_vline(xintercept = 0,linetype="dashed",color="black",size = 2)+
    coord_cartesian(ylim = c(0,100),xlim = c(-12,12))+
    scale_y_continuous(breaks = seq(0,100,20))+
    scale_x_continuous(breaks = seq(-12,12,2))+
    labs(x = "Time from susnet [h]",
         y = "Gaping Percentage [%]") +
    ...
    theme(panel.grid.major = element_blank(),
        ...
        panel.background = element_blank(), 
        axis.line = element_line(colour = "black"),
        strip.background = element_rect(fill="white",color="black", linewidth = 2.5, linetype="solid"),
        axis.title.y = element_text(size = 20),
...
Day_Night_gaping_graph_full + inset_element(Day_Night_gaping_graph_zoom,left = 0.65,bottom = 0.04,right = 1,top = 0.32)
```

##Correlations plot - bivalves and environmental
```{r}

#Bivalve correlation graph 
biv_2min_correlation_hitsogram <- ggplot(bivalve_corr, aes(corr_values)) +
...
biv_2min_correlation_boxplot <- ggplot(bivalve_corr, mapping =  
                       aes(corr_values))+
                       geom_boxplot(aes(y=corr_values,x=NULL),fill =
                                      "antiquewhite1", color = "black")+
                       stat_summary(mapping = aes(y = corr_values, x=0),fun =
                                      mean,geom="point",color="blue", shape = 18,size =4)+
...
biv_2min_corr_plot <- biv_2min_correlation_boxplot/biv_2min_correlation_hitsogram+
             plot_layout(heights = c(0.2,4))+
             plot_annotation(title = 'Bivalves Correlations 2 min')

biv_1hour_correlation_hitsogram <- ggplot(biv_corr_hour, aes(corr_values)) +
                           geom_histogram(binwidth = 0.1,breaks = seq(-1, 1, by = 0.1),
                           ...
                           axis.line.y = element_line(colour = "black"),
                           axis.line.x = element_line(colour = "black"))
biv_1hour_correlation_boxplot <- ggplot(biv_corr_hour, mapping =  
                       aes(corr_values))+
                       geom_boxplot(aes(y=corr_values,x=NULL),fill =
                                                        "lightpink", color = "black")+
                       stat_summary(mapping = aes(y = corr_values, x=0),fun =
                                      mean,geom="point",color="blue", shape = 18, size = 4)+
                       scale_y_continuous(limits = c(-1,1),breaks =seq(-1, 1, by = 
                                          0.2),minor_breaks = seq(-1, 1, by = 0.1) )+
                       ...
                       axis.title.y = element_blank(),
                       axis.text.y = element_blank())
biv_1hour_corr_plot <- biv_1hour_correlation_boxplot/biv_1hour_correlation_hitsogram+
             plot_layout(heights = c(0.2,4))+
             plot_annotation(title = 'Bivalves Correlations 1 hour')

biv_1day_correlation_hitsogram <- ggplot(biv_corr_day, aes(corr_values)) +
                           geom_histogram(binwidth = 0.1,breaks = seq(-1, 1, by = 0.1),
                                          fill = "coral1", color = "black") +
                           scale_x_continuous(limits = c(-1,1),breaks =seq(-1, 1, by = 
                                            0.2),minor_breaks = seq(-1, 1, by = 0.1))+
                           scale_y_continuous(limits = c(0,40))+
                           labs(x = "Correlation values",y = "Count")+
                           ...
                           axis.line.y = element_line(colour = "black"),
                           axis.line.x = element_line(colour = "black"))
biv_1day_correlation_boxplot <- ggplot(biv_corr_day, mapping =  
                       aes(corr_values))+
                       geom_boxplot(aes(y=corr_values,x=NULL),fill =
                                                        "coral1", color = "black")+
                       stat_summary(mapping = aes(y = corr_values, x=0),fun =
                       ...
                       axis.title.y = element_blank(),
                       axis.text.y = element_blank())
biv_1day_corr_plot <- biv_1day_correlation_boxplot/biv_1day_correlation_hitsogram+
             plot_layout(heights = c(0.2,4))+
             plot_annotation(title = 'Bivalves Correlations 1 day')
biv_corr_plot <- ggarrange(biv_2min_corr_plot,biv_1hour_corr_plot,biv_1day_corr_plot,nrow = 3)

#bivalve correlation violin
res_order <- c("2min","1hour","1day")
biv_cor_res_violin <- ggplot(biv_corr_all_res,mapping = aes(x=factor(res,res_order),y=corr_values))+
  geom_violin(fill="firebrick2")+
  geom_hline(yintercept = 0, linetype = "dashed", color = "black", 
                                      size = 1)+
  coord_flip()+
  geom_point(data = biv_corr_sum,mapping = aes(x=res,y=mean),color="blue",shape=18,size=4)+
  geom_errorbar(data = biv_corr_sum,mapping = aes(x=res,y=mean,ymin=mean-conf_int, ymax=mean+conf_int),width=0.1,color="blue")+
  labs(x="Resolution",y="Correlation Values")+
...
##Opening intervals closing defintion boxplot plot
```{r}
bivalves_opening_times$limit <- as.factor(bivalves_opening_times$limit )
opening_interval_sum$limit <-  as.factor(opening_interval_sum$limit )
opening_interval_diff_closing_prec_plot <-        
                       ggplot(bivalves_opening_times,aes(x=limit,y=mean.opening.time))+
                       geom_boxplot(width=0.4, color="black",fill="firebrick2") +
                       geom_point(data = opening_interval_sum,mapping = 
                                    aes(y=biv_open_interval_mean),color="blue",shape=18,size=4)+
                       geom_errorbar(data = opening_interval_sum,mapping = 
                         aes(y=biv_open_interval_mean,ymin=biv_open_interval_mean-conf_int_open_interval,
                             ymax=biv_open_interval_mean+conf_int_open_interval),width=0.1,color="blue")+
