...
##check mean and max opening time for in 3 hours bins and continous
##3hours bins
dblong_3h_bin <- dblong2 %>% select(-Sunset,-Sunrise)
dblong_3h_bin$Day_bin <- cut(dblong_3h_bin$group, breaks= "1 day")
#chose 20 just to see if works
close_limit <- 20
dblong_3h_bin$DayofYear <- yday(dblong_3h_bin$group)
dblong_3h_bin$Season <- GetSeason(dblong_3h_bin$DayofYear)
dblong_3h_bin$open_close <- ifelse(dblong_3h_bin$biv_open_interval_perc>close_limit,"Open","Close")
bivalves_3h_open_time_ratio_and_perc <- data.frame(
  bivalve_name=character(),
  date=Date(),
  hour=character(),
  open_time_ratio=double(),
...
for (biv in bivalves_names) {
  temp_biv <- dblong_3h_bin %>% filter(Biv_num==biv)
  temp_dates_list <-unique(temp_biv$Day_bin)
  for (date in temp_dates_list) {
    temp_date <- temp_biv %>% filter(Day_bin==date)
    temp_date$ThreeHour_bin <-  cut(temp_date$group, breaks= "3 hours")
    temp_date <- temp_date %>% group_by(ThreeHour_bin) %>%
    ...
    bivalves_3h_open_time_ratio_and_perc <- rbind(bivalves_3h_open_time_ratio_and_perc,temp_date)
  }
}
rm(temp_date,temp_biv)
write.csv(bivalves_3h_open_time_ratio_and_perc,"Bivalves_3h_open_time&perc_ratio.csv")
sum_bivalves_3h_open_time_ratio_and_perc <- bivalves_3h_open_time_ratio_and_perc %>% 
  group_by(hour,Season) %>% 
  summarise_at(vars(open_time_ratio,mean_open_perc), funs(mean, median, sd, n())) %>% 
  mutate(conf_int = sd/sqrt(n) * qt(p = 0.975, df = n-1))
write.csv(sum_bivalves_3h_open_time_ratio,"Bivalves_3h_open_time_ratio_sum.csv")
...
#bivalve correlation boxplot
res_order <- c("2min","1hour","1day")
biv_cor_res_boxplot <- ggplot(biv_corr_all_res,mapping = aes(x=factor(res,res_order),y=corr_values))+
